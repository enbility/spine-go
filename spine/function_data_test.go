...
func TestFunctionData_UpdateData(t *testing.T) {
	...
	_, _ = sut.UpdateData(false, true, newData, nil, nil, nil)
	...
	_, _ = sut.UpdateDataAny(false, true, newData, nil, nil, nil)
...
func TestFunctionData_UpdateDataPartial(t *testing.T) {
	...
	_, err := sut.UpdateData(false, true, newData, &model.FilterType{CmdControl: &model.CmdControlType{Partial: &model.ElementTagType{}}}, nil, nil)
	if assert.Nil(t, err) {
		getData := sut.DataCopy()
